...
wczytaj_baze <- reactive({
  ...
  if (is.null(baza_rolnikow())) {
    if (file.exists("Baza_danych/Baza/baza_rolnikow.qs")) {
      baza_rolnikow <- qread("Baza_danych/Baza/baza_rolnikow.qs", nthreads = 4L)
      baza_rolnikow <- as.data.table(baza_rolnikow)
      ...
      qsave(baza_rolnikow, "Baza_danych/Baza/baza_rolnikow.qs", preset = "fast", nthreads = 4L)
      baza_rolnikow(baza_rolnikow)
    }
  }
})
...
stworz_kopie_automatyczne <- reactive({
    ...
    if (as.numeric(as.duration(interval(ostatnia_kopia, obecna_data)), "days") >= 14) {
      if (file.exists("Baza_danych/Baza/baza_rolnikow.qs")) {
        ...
        file.copy("Baza_danych/Baza/baza_rolnikow.qs", stri_c("Baza_danych/Kopia_automatyczna/", obecna_data))
      }
    }
...
observe(stworz_kopie_automatyczne())
 ...
 odejmuj_dni_zawieszenia <- reactive({
   ...
   if (data_z_pliku < data_dzisiejsza) {
     ...
     qsave(baza_rolnikow_odejmuj, "Baza_danych/Baza/baza_rolnikow.qs", preset = "fast", nthreads = 4L)
...
data_kopii_uzytkownika <- reactive({
  invalidateLater(100)
  as.character(file.mtime("Baza_danych/Kopia_uzytkownika/baza_rolnikow.qs"))
})
...
server <- function(input, output, session) {
    ...
    observeEvent(input$zapisuj_kopie_przycisk, {
        withProgress(message = "Zapisywanie", file.copy("Baza_danych/Baza/baza_rolnikow.qs", "Baza_danych/Kopia_uzytkownika/baza_rolnikow.qs", overwrite = TRUE))
    })
    
    observeEvent(input$przywracaj_kopie_uzytkownika_przycisk, {
        withProgress(message = "Przywracanie", {
          file.copy("Baza_danych/Kopia_uzytkownika/baza_rolnikow.qs", "Baza_danych/Baza/baza_rolnikow.qs", overwrite = TRUE)
          baza_rolnikow <- qread("Baza_danych/Baza/baza_rolnikow.qs", nthreads = 4L)
          baza_rolnikow <- as.data.table(baza_rolnikow)
          setkey(baza_rolnikow, telefon_0)
          baza_rolnikow(baza_rolnikow)
    ...
    observeEvent(input$przywracaj_kopie_automatyczne_przycisk,{
        if (input$daty_kopii_automatycznych_wybor != "Wybierz datę") {
            withProgress(message = "Przywracanie", {
              file.copy(stri_c("Baza_danych/Kopia_automatyczna/", input$daty_kopii_automatycznych_wybor, "/", "baza_rolnikow.qs"), "Baza_danych/Baza/baza_rolnikow.qs", overwrite = TRUE)
              baza_rolnikow <- qread("Baza_danych/Baza/baza_rolnikow.qs", nthreads = 4L)
              baza_rolnikow <- as.data.table(baza_rolnikow)
              setkey(baza_rolnikow, telefon_0)
              baza_rolnikow(baza_rolnikow)
